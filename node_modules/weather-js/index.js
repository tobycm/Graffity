...
module.exports = (function() {
  ...
  var find = function find(options, callback) {
    ...
    if(!options || typeof options !== 'object')
      return callback('**🚫 |** Tùy chỉnh không hợp lệ!');

    if(!options.search)
      return callback('**🚫 |** Xin hãy ghi nơi!');
    ...
    request.get({url: reqUrl, timeout: timeout}, function(err, res, body) {
      ...
      if(res.statusCode !== 200) return callback(new Error('**🚫 |** Yêu cầu thất bại (' + res.statusCode + ')'));
      if(!body)                  return callback(new Error('**🚫 |** Tìm dữ liệu thất bại'));
      ...
      xmlParser.parseString(body, function(err, resultJSON) {
        ...
        if(!resultJSON || !resultJSON.weatherdata || !resultJSON.weatherdata.weather)
          return callback(new Error('**🚫 |** Không thể in data!'));
        ...
        if(!(resultJSON.weatherdata.weather instanceof Array)) {
          return callback(new Error('**🚫 |** Thiếu thông tin!'));
        }
